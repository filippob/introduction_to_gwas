...
snakemake --dag  --snakefile example_snakefile | dot -Tsvg > dag_example.svg
...
snakemake --dag --snakefile example_snakefile | dot -Tsvg > dag_example_unzipped.svg

## rerun the first example pipeline: actual run for DAG
snakemake --cores 1 --snakefile example_snakefile

## run the second example pipeline: dry run for DAG
snakemake --dag --snakefile example_snakefile_2 | dot -Tsvg > dag_example_2.svg
