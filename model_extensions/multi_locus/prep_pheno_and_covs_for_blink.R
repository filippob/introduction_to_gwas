...
fwrite(x = as.matrix(covariates[,-1]), file = fname, sep = "\t", col.names = FALSE)

writeLines(" - writing out the phenotype file")
# temp = unlist(strsplit(dirname(idsf), split = "/")[[1]])
# path_to_folder = here(paste(temp[temp != ".."], collapse = "/"))
path_to_folder = dirname(idsf)
fname = file.path(path_to_folder, "phenotypes.txt")
fwrite(x = select(rice, c(id,PH)), file = fname, sep = "\t", col.names = TRUE)
