...
# run as Rscript --vanilla gwas_rrblup.R genotype_file="rice_imputed.raw" snp_map="rice_imputed.map" phenotype_file="rice_phenotypes.txt" trait="FLW" trait_label="phenotype"
...
## Running the GWAS
###################
print("Running the GWAS")

df <- phenotypes[,-c(1:3)] ## remove ID column
...
for(i in 1:ncol(matg)) {
  ...
  fit <- glm(phenotypes$FLW ~ ., data = df, family = "gaussian")
